...
(function( $ ) {
  ...
  var methods = {
      ...
      return this.each(function(){
        ...
        var $this = $(this),
            ...
            timeline = $('<div />', {
              ...
              "show-headline"         : settings.showHeadline ? 1 : '',
        ...
        // $this.on( 'afterAddEvent.timeline', '.timeline-events', methods.afterAddEvent);
        $this.on( 'afterRender.timeline', function(){
          $(this).off('afterRender.timeline');
          methods.afterAddEvent(settings.afterAddEvent,  $(this).data('timeline'));
          // $this.trigger( 'afterAddEvent.timeline', [ settings.afterAddEvent ] );
        });
    ...
    afterAddEvent: function(callback, data){
      var clicking = false;
      var constWidth = 30;
      var newEl;
      if (typeof callback !== "function") return false;
      $(document).on("mousedown mouseup mousemove", ".timeline-events", function (e) {
        var offset = {
          X: $(".timeline-events").offset().left,
          Y: $(".timeline-events").offset().top
        }
        if (e.target.classList.contains('timeline-events')) {
            $('.timeline-timetable.timeline-grids').trigger(e.type, [e, offset]);
            // console.log("layerX:" + e.originalEvent.layerX + " clientX:" + e.clientX + " offsetX:" + e.offsetX);
            // console.log("layerY:" + e.originalEvent.layerY + " clientY:" + e.clientY + " offsetY:" + offsetY);
        } else {
            $('.timeline-timetable.timeline-grids').trigger(e.type, [e, offset]);
            // console.log("clientX:" + e.clientX + " offsetX:" + e.offsetX);
        }
      });
      $(document).on("mousedown", '.timeline-timetable.timeline-grids', function (eTriggered, e , offset) {
          if (e.which !==1) return false;
          clicking = true;
          newEl = $('<div />', { addClass: "timeline-node timeline-text-truncate", text: "simple" });
          var yCord = (((e.clientY - offset.Y) / 40) | 0);
          var xCord = (((e.clientX - offset.X) / constWidth) | 0);
          newEl.css({ "top": yCord * 40 + "px", "left": xCord * constWidth + "px", "width": constWidth * 1 + "px" });
          $(".timeline-events").append(newEl);
      });

      $(document).on("mouseup", '.timeline-timetable.timeline-grids', function (eTriggered, e, offset) {
          clicking = false;
          if (newEl) {
            var timelineDate = new Date(data.timeline.attr( 'start-datetime'));
            var xStart = (((+newEl.css("left").replace("px", "")) / constWidth) | 0);
            var xEnd = (((e.clientX - offset.X) / constWidth) | 0) + 1;
            var startDate = new Date(timelineDate.getTime() + timelineDate.getTimezoneOffset()*60000 + xStart*constWidth*60000);
            var endDate = new Date(timelineDate.getTime() + timelineDate.getTimezoneOffset()*60000 + xEnd*constWidth*60000);



            callback(startDate.toJSON(), endDate.toJSON());
          }
          newEl = null;
      });

      $(document).on("mousemove", '.timeline-timetable.timeline-grids', function (eTriggered, e, offset) {
          if (clicking == false) {
            newEl = null;
          }
          if (newEl) {
              var prevX = +newEl.css("left").replace("px", "");
              var currX = (e.clientX - offset.X);
              var diff = ((currX - prevX) / constWidth | 0) + 1;
              // console.log("currX:" + currX + "prevX:" + prevX + "diff:" + diff + "width:" + constWidth * diff);
              newEl.css({ "width": constWidth * diff + "px" });
          }
          var target = e.target;
